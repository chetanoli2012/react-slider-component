...
const SlideShow = ({ images }) => {
  ...
  const handleBulettedNavigation = (imageIndex) => {
    setActive(imageIndex);
  };

  const hanldeNext = () => {
    if (active < images.length - 1) {
      setActive((prevVal) => prevVal + 1);
    } else {
      ...
      {images.map(function (image, imageIndex) {
        return (
          <Slide
            {...image}
            key={image.caption}
            active={active === imageIndex}
          />
        );
      })}
      <div className="buletted-navigation">
        {images.map(function (image, imageIndex) {
          return (
            <div
              className={`dot ${active === imageIndex ? "active" : ""}`}
              key={image.caption}
              onClick={() => handleBulettedNavigation(imageIndex)}
            />
          );
        })}
      </div>
      <div className="next-prev-navigation">
        <div className="navigation next" onClick={hanldeNext}>
          &gt;
        </div>
        <div className="navigation prev" onClick={hanldePrev}>
          &lt;
        </div>
      </div>
    </div>
...
const Slide = ({ image_url, caption, active }) => {
  return (
    <div className={`slide ${active ? "active" : ""}`}>
      <img src={image_url} alt={caption}></img>
      <span>{caption}</span>
    </div>
  );
};
